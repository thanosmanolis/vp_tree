...
vptree * buildvp_recursive(double *X, int n, int d, int *idx_list)
{
    ...
    if(n > 1)
    {
        ...
        if(n>2)
        {
            ...
            T->outer = cilk_spawn buildvp_recursive(outer, outer_n, d, outer_idx);
        }else
            T->outer = cilk_spawn buildvp_recursive(outer, 0, d, outer_idx);

        T->inner = cilk_spawn buildvp_recursive(inner, inner_n, d, inner_idx);
        cilk_sync;
